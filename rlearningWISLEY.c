...
void anda(int ambiente[][6],float q[][6][4],agente *r,float *ro);
float AcaoAle( float q[][6][4], int i, int j,int *pos, int ambiente [][6]);
...
if (cont > 1){
	maior2 = q[i][j][indice[0]];
	p2 = indice[0];
	if (maior == maior2){
		maior2 = q[i][j][indice[1]];
		p2 = indice[1];
	}
	for (k = 0; k < cont; k++){	//procura a acao de maior valor.
		
		if(maior2 < q[i][j][indice[k]] && maior2 != maior){
			maior2 = q[i][j][indice[k]];
			p2 = indice[k];
		}
	}
...
void anda (int ambiente[][6],float q[][6][4],agente *r,float *ro){
	...
	if(ran < 1-epsilon){
		...
		float acao = AcaoAle(q,r->linha,r->coluna,&pos,ambiente);
	}
	...
	switch(pos){
		case UP:{
			...
			q[r->linha][r->coluna][UP] =q[r->linha][r->coluna][UP]+alfa*(reco-*ro+acao2-q[r->linha][r->coluna][UP]);
			float acao3 = melhorAcao(q,r->linha,r->coluna,&p,ambiente);
			if (q[r->linha][r->coluna][UP] == acao3){
				acao2 =  melhorAcao(q,r->linha-1,r->coluna,&pos,ambiente);
				*ro = *ro + sigma*(reco - *ro + acao2 - acao3);
			}
			r->linha = r->linha - 1;
			break;
		}
		case DOWN:{
			printf(" DOWN \n");
			acao2 = melhorAcao(q,r->linha + 1,r->coluna,&pos,ambiente);
			q[r->linha][r->coluna][DOWN] =q[r->linha][r->coluna][DOWN]+alfa*(reco-*ro+acao2-q[r->linha][r->coluna][DOWN]);
			float acao3 = melhorAcao(q,r->linha,r->coluna,&p,ambiente);
			printf("%d\n", p );
			if ((q[r->linha][r->coluna][DOWN] == acao3)){
				acao2 = melhorAcao(q,r->linha + 1,r->coluna,&pos,ambiente);
				*ro = *ro + sigma*(reco - *ro + acao2 - acao3);
		...
		case LEFT:{
			...
			q[r->linha][r->coluna][LEFT] =q[r->linha][r->coluna][LEFT]+alfa*(reco-*ro+acao2-q[r->linha][r->coluna][LEFT]);
			float acao3 = melhorAcao(q,r->linha,r->coluna,&p,ambiente);
			if ((q[r->linha][r->coluna][LEFT] == acao3)){
				acao2 =  melhorAcao(q,r->linha,r->coluna-1,&pos,ambiente);
				*ro = *ro + sigma*(reco - *ro + acao2 - acao3);
			}
			r->coluna = r->coluna - 1;
		...
		case RIGHT:{
			printf(" RIGHT \n");
			acao2 =  melhorAcao(q,r->linha,r->coluna+1,&pos,ambiente);
			q[r->linha][r->coluna][RIGHT] =q[r->linha][r->coluna][RIGHT]+alfa*(reco-*ro+acao2-q[r->linha][r->coluna][RIGHT]);
			float acao3 = melhorAcao(q,r->linha,r->coluna,&p,ambiente);
			if ((q[r->linha][r->coluna][RIGHT] == acao3)){
				acao2 =  melhorAcao(q,r->linha,r->coluna+1,&pos,ambiente);
				*ro = *ro + sigma*(reco - *ro + acao2 - acao3);
			}
			r->coluna = r->coluna + 1;
